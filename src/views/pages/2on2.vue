<template>
  <div>
    <h1 class="standings-heading">STANDINGS <span>2 on 2</span></h1>
    ...
    <div class="ft-controls">
      ...
      <button class="ft-filter-btn ft-advanced-toggle" :class="{ 'active-gold': showAdvanced || hasActiveAdvanced }"
      ...
      <div v-if="showAdvanced" class="adv-panel">
        <div class="adv-panel-inner">
          ...
          <div class="adv-field">
            ...
            <div class="range-wrap">
              <input type="range" v-model.number="filters.minBattles" :min="0" :max="maxBattlesInData"
                class="ft-range" />
              <div class="range-track-fill" :style="{ width: rangePercent(filters.minBattles, 0, maxBattlesInData) }">
              </div>
            </div>
          </div>
          ...
          <div class="adv-field">
            ...
            <div class="range-wrap">
              <input type="range" v-model.number="filters.minWins" :min="0" :max="maxWinsInData" class="ft-range" />
            </div>
          </div>
    ...
    <div v-if="!loading" class="results-meta">
      <span class="results-count">
        {{ filtered.length }}
        <span class="results-label">{{ filtered.length === 1 ? 'tandem' : 'tandems' }}</span>
      </span>
      ...
      <table class="standings-table">
        ...
        <tbody>
          <template v-if="filtered.length">
            <tr v-for="(tandem, i) in filtered" :key="tandem.Name" class="data-row">
              <!-- Rank -->
              <td class="rank-cell">
              ...
              <!-- Name -->
              <td class="tandem-cell">
                {{ tandem.Name }}

              </td>

              <td class="win-cell">{{ tandem.Win }}</td>
              <td class="loss-cell">{{ tandem.Loss }}</td>
              <!-- <td class="stat-cell">{{ tandem.Total }}</td> -->
                ...
                <div class="winrate-bar-wrap">
                  <div class="winrate-bar">
                    <div class="winrate-bar-fill" :style="{
                      width: rawRate(tandem) + '%',
                      background: winRateColor(rawRate(tandem)),
                    }"></div>
                  </div>
                  <span class="winrate-label" :style="{ color: winRateColor(rawRate(tandem)) }">
                    {{ rawRate(tandem) }}%
                  </span>
                </div>
              </td>
...
import { use2on2 } from '@/composables/2on2'
